...
### 自定义polly Pass
#### 1.MatAddParallelPass.cpp
1.
../build/bin/clang -O3 -S -emit-llvm matadd.c -o matadd.ll -mllvm -polly -mllvm -polly-process-unprofitable

2.
../build/bin/opt  -matadd-parallel -S matadd.ll -o matadd_opt.ll -debug
../build/bin/opt -matadd-parallel -S matadd.ll -o matadd_opt.ll -debug-only=matadd-parallel


#### 2.MatMulTilingPass.cpp
1.
../build/bin/clang -c -emit-llvm -S matmul_MatMulTilingPass.c -o matmul_mmtp.ll -O1

../build/bin/clang -c -emit-llvm -S -DTEST matmul.c -o matmul_mmtp.ll -O1

2.
../build/bin/opt -matmul-tiling -S matmul_mmtp.ll -o matmul_mmtp_opt.ll -debug-only=matmul-tiling -debug
...
../build/bin/opt \
  ...
  -polly-codegen \
  -polly-cleanup \
  -S matmul_mmtp.ll -o matmul_mmtp_codegen.ll


4.
../build/bin/lli matmul_mmtp_codegen.ll




cd /home/sunteng/codespace/llvm-project/polly-demo
../build/bin/clang -S -emit-llvm matmul_MatMulTilingPass.c -Xclang -disable-O0-optnone -o matmul_mmtp.ll
...
```
../build/bin/opt -S -polly-canonicalize matmul_mmtp.ll  -o matmul_mmtp.preopt.ll

```

#### 3.请显示Polly检测到的SCoP
../build/bin/opt --basicaa -polly-ast -analyze matmul_mmtp.preopt.ll -polly-process-unprofitable -polly-use-llvm-names


# 1. 生成 LLVM IR（matmul.c 内容为你提供的矩阵乘法）
../build/bin/clang -O1 -emit-llvm -S matmul.c -o matmul.ll

# 2. 应用 pass（无 -polly-fusion 参数）
../build/bin/opt -polly-canonicalize -polly-scops -matmul-tiling -S matmul.ll -o matmul_tiled.ll -debug-only=matmul-tiling -debug
